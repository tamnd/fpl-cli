...
describes what users get. Pick a format with `-o`, or let fpl choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
fpl <command> -o table   # aligned columns for reading
fpl <command> -o jsonl   # one JSON object per line, for piping
fpl <command> -o json    # a single JSON array
fpl <command> -o csv     # spreadsheet friendly
fpl <command> -o tsv     # tab-separated
fpl <command> -o url     # just the URL column
fpl <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
fpl <command> --fields id,title,url
```
...
```bash
fpl <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
fpl <command>            # a table, because this is a terminal
fpl <command> | wc -l    # JSONL, because this is a pipe
```
