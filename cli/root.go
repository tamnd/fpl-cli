// Package cli assembles the fpl command tree from the fpl
...
// NewApp assembles the kit application from the fpl domain. The
...
//
// To add a command, declare it in fpl/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := fpl.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(fpl.Domain{}).Register(app)
